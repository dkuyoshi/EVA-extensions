...
class QNet(Chain):
    def __init__(self, n_history, num_actions, n_atoms, v_min, v_max, n_hidden=256):
        super().__init__()
        with self.init_scope():
            self.hout = CNN(n_history, n_hidden)
            self.qout = L.Linear(n_hidden, num_actions * n_atoms)
...
class DistributionalQFunction(Chain):
    def __init__(self, n_history, num_actions, xp, LRU, n_atoms, v_min, v_max, n_hidden=256,
                 lambdas=0.5, capacity=2000, num_neighbors=5):
        super().__init__()
        with self.init_scope():
            self.q_func = QNet(n_history, num_actions, n_atoms, v_min, v_max, n_hidden)
...
class DuelingQNet(Chain):
    def __init__(self, n_history, num_actions, n_atoms, n_hidden=256):
        super().__init__()
        with self.init_scope():
            self.hout = CNN(n_history, n_hidden)
            # self.fully_layer = L.Linear(n_hidden, 512)
            self.a_stream = L.Linear(n_hidden, num_actions * n_atoms)
            self.v_stream = L.Linear(n_hidden, n_atoms)
...
class DistributonalDuelingQFunction(Chain):
    def __init__(self, n_history, num_actions, xp, LRU, n_atoms, v_min, v_max, n_hidden=256,
                 lambdas=0.5, capacity=2000, num_neighbors=5):
        super().__init__()
        with self.init_scope():
            self.q_func = DuelingQNet(n_history, num_actions, n_atoms, n_hidden)
        self.non_q = ValueBuffer(capacity, num_neighbors, n_hidden, num_actions, xp=xp, LRU=LRU)
        self.lambdas = lambdas
        self.num_actions = num_actions
        self.capacity = capacity
    ...
    def __call__(self, x, eva=False, test=False):
        ...
        self.embedding = self.get_embedding()
        if not eva or self.lambdas == 0 or self.lambdas == 1:
            return DistributionalDiscreteActionValue(F.softmax(q), self.z_values)
        qnp = self.non_q.get_q(self.embedding.array)
        qout = self.lambdas * q.q_values + (1 - self.lambdas) * qnp
        return DiscreteActionValue(qout)
