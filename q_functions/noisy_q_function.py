...
class NoisyCNN(Chain):
    def __init__(self, n_history=4, n_hidden=256, sigma=0.5):
        super().__init__()
...
class NoisyQNet(Chain):
    def __init__(self, n_history, num_actions, n_hidden=256, sigma=0.5):
        super().__init__()
        with self.init_scope():
            self.hout = NoisyCNN(n_history, n_hidden, sigma)
            self.qout = FactorizedNoisyLinear(L.Linear(n_hidden, num_actions), sigma)

    def __call__(self, x, test=False):
        self.embedding = self.hout(x)
        return DiscreteActionValue(self.qout(F.relu(self.embedding)))

class NoisyQFunction(Chain):
    def __init__(self, n_history, num_actions, xp, LRU, n_hidden=256, lambdas=0.5, capacity=2000, num_neighbors=5, sigma=0.5):
        super().__init__()
        with self.init_scope():
            self.q_func = NoisyQNet(n_history, num_actions, n_hidden, sigma)
...
class NoisyDuelingQNet(Chain):
    def __init__(self, n_history, num_actions, n_hidden=256, sigma=0.5):
        ...
        with self.init_scope():
            ...
            self.a_stream = FactorizedNoisyLinear(L.Linear(n_hidden, num_actions), sigma)
            self.v_stream = FactorizedNoisyLinear(L.Linear(n_hidden, 1), sigma)
